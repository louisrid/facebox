...
import { ArrowRight, Loader2, RefreshCw, Gem } from "@/lib/icons";
...
const BouncingEmoji = ({ emoji }: { emoji: string }) => (
  ...
  >
    <span className="inline-block text-[60px] md:text-[78px] leading-none" style={{ animation: "emoji-bounce 1.6s ease-in-out infinite" }}>
      {emoji}
    </span>
  </div>
);
...
const GuidedCreator = forwardRef<HTMLDivElement, GuidedCreatorProps>(({ open, onComplete, onExit, skipWelcome = false, resumeAtCreate = false }, _ref) => {
  ...
  }, [open, flowVariant, resumeAtCreate, flowSteps.length]);
  ...
  useEffect(() => {
    ...
    setHeroPhase(0);
    const ts = [
      setTimeout(() => setHeroPhase(1), 50),
      setTimeout(() => setHeroPhase(2), 100),
      setTimeout(() => { setHeroPhase(3); heroVisited.current = true; }, 200),
    ];
  ...
  const renderHero = () => {
    ...
    return (
      <div className="flex w-full flex-col items-center justify-center" style={{ minHeight: '100vh', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, fontSize: 108, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.03em', lineHeight: 1, marginTop: -14, marginBottom: 38 }}>facebox</div>
  ...
  return createPortal(
    ...
    >
          ...
          <div className="flex items-center justify-center gap-[6px] md:gap-[8px] mx-auto">
            {Array.from({ length: dashCount }).map((_, i) => (
              <div key={i} className="transition-all duration-300" style={{
                width: 10, height: 10, borderRadius: 8, flexShrink: 0,
      ...
      <div className="flex-1 flex justify-center px-6 md:px-12 min-h-0 overflow-hidden">
        <div
          className={`mx-auto flex w-full flex-col max-w-sm md:max-w-lg ${visualStepType === "hero" ? "items-center justify-center" : "items-center justify-center"}`}
          style={visualStepType === "hero" ? undefined : { paddingTop: 0, paddingBottom: 0 }}
        >
          {isHeroSlide ? (
...
export const SignupSlide = ({ open, onSignedIn }: { open: boolean; onSignedIn: () => void }) => {
  ...
  const handleBack = () => {
    ...
    navigate("/", { state: { openCreator: true, resumeAtCreate: true } });
  };
