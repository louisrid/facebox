...
const VideoIcon = ({ size = 24, className }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
    <path d="M3 12C3 16.9706 7.02944 21 12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12Z" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M10 15V9L15 12L10 15Z" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const BottomTabBar = () => {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-[9000] flex h-[84px] md:hidden"
      style={{
    ...
    >
      {TABS.map((tab) => {
        ...
        return (
          ...
          >
            <Icon size={24} />
            <span className="text-[12.5px] font-[800] lowercase tracking-wide">{tab.label}</span>
          </button>
        );
      })}
    </nav>
