...
import { Loader2, Users, ImageIcon, Sparkles, ArrowLeft, Download, User, Copy } from "@/lib/icons";
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/95 px-4 pt-24 pb-6">
      <div className="relative w-full max-w-md md:max-w-lg">
        ...
        <div>
          <img src={photo.image_url} alt="" className="w-full rounded-[8px] object-contain max-h-[60vh]" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 max-w-lg">
        ...
        <div style={{ backgroundColor: "hsl(var(--card))", borderRadius: 8 }} className="px-4 py-3">
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ padding: "12px 0", color: "#ff4444", borderRadius: 8, border: "2px solid #ff4444", backgroundColor: "transparent" }}
      >
        reset user
      </button>
                    ...
                    {photos.length > 0 ? (
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 md:gap-4">
                        {photos.map((p: any, i: number) => {
                          ...
                          return (
                            <button key={i} onClick={() => setSelectedPhoto(p)} className="text-left hover-lift" style={{ borderRadius: 8, backgroundColor: "hsl(var(--card))", overflow: "hidden" }}>
                              ...
                              <div className="px-3 py-2.5 space-y-1">
                                {type === "system" ? (
                                  <span className="inline-block text-[9px] md:text-[10px] font-[800] lowercase px-2 py-0.5" style={{ borderRadius: 8, backgroundColor: "hsl(var(--card))", color: "#ffffff" }}>
                                    {label}
                                  </span>
                                ) : (
                    ...
                    {rejected.length > 0 ? (
                      <div className="space-y-1.5">
                        {rejected.map((r: any, i: number) => (
                          <div key={r.id || i} className="px-3.5 py-2.5 md:py-3" style={{ borderRadius: 8, backgroundColor: "hsl(var(--card))" }}>
