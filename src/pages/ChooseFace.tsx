...
const ChooseFace = () => {
  ...
  useEffect(() => {
    if (barComplete && apiDone && faces.length > 0) {
      setLoading(false);
    }
  }, [barComplete, apiDone, faces.length]);
  ...
  const generateFaces = async () => {
    ...
    try {
      ...
      let result = await invokeGenerate({ prompt, face_regen: true });

      if (result?.error && result.code === "FREE_GEN_USED") {
        result = await invokeGenerate({ prompt, face_regen: true });
      }
      ...
      {/* Face picker — shown after loading completes */}
        {!loading && faces.length > 0 && (
          <>
            ...
            <main className="relative z-[1] mx-auto flex w-full max-w-lg flex-col overflow-y-auto px-[24px] pt-[34px] pb-[160px] md:max-w-3xl md:px-10">
              ...
              <div className="mx-auto flex w-full max-w-[26rem] md:max-w-[40rem] flex-col gap-4 md:gap-6">
                ...
                {(() => {
                  try {
                    ...
                    return (
                      <div className="mt-4 md:mt-6 px-4 py-3" style={{ backgroundColor: "hsl(var(--card))", borderRadius: 8 }}>
            ...
            <div className="mt-16 flex flex-col items-center gap-4">
              ...
              {generationError && (
                <button
                  ...
                  style={{ backgroundColor: "#ffffff", color: "#000000", borderRadius: 8 }}
                >
                  try again
                </button>
              )}
